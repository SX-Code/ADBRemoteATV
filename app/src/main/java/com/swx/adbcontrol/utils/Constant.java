package com.swx.adbcontrol.utils;

/**
 * @Author sxcode
 * @Date 2024/5/17 13:52
 */
public class Constant {
    public static final String KEY_SETTING_LAYOUT_NAV = "key_setting_layout_nav";
    public static final String KEY_SETTING_LAYOUT_QUICK_ACCESS = "key_setting_layout_quick_access";
    public static final String KEY_SETTING_BEHAVIOR_HAPTIC_FEEDBACK = "key_setting_behavior_haptic_feedback";
    public static final String KEY_QUICK_ACCESS_ORDER_CHANGE = "key_quick_access_order_change";
    public static final String KEY_QUICK_ACCESS_APP_ONLINE_URL = "key_quick_access_app_online_url";
    public static final String DEFAULT_QUICK_ACCESS_APP_ONLINE_URL = "https://gitee.com/SX-Code/adbremote-atv/raw/master/apps.json";
    public static final String VALUE_ACTIVITY_APP_PRIORITY = "value_activity_app_priority";
    public static final String URL_PREFIX_CN = "https://gitee.com/SX-Code/adbremote-atv/raw/master/";
    public static final String URL_PREFIX_EN = "https://gitee.com/SX-Code/adbremote-atv/raw/master/";
    public static final String URL_LATEST_APK_INFO_EN = URL_PREFIX_EN + "/latest_apk_info.txt";
    public static final String URL_LATEST_APK_INFO_CN = URL_PREFIX_CN + "latest_apk_info.txt";
    public static final String URL_CHECK_VERSION = "";
    public static final String SAVE_FILE_NAME = "ADB_Remote_ATV_%s.apk";
    public static final String URL_GITHUB_CODE_PAGE = "https://github.com/SX-Code/ADBRemoteATV";
    public static final String URL_GITHUB_FEEDBACK_PAGE = "";
}
